<template>
  <div class="relative">
    <video
      ...
      class="video"
      muted
      :src="'/videos/' + theme + '/Corner_Score.webm'">
      <!-- :height="orientaion=='landscape' ? '1080' : '1920' "
      :width="orientaion=='landscape' ? '1920' : '1080' " -->
    </video>

    <div class="left-[976px] teamWrap" ref="team1wrapper">
      <img
        :src="'/images/' + theme + '/zat_white.svg'"
        class="absolute w-[66px] h-[62px] -top-0.5 left-64" />
      <transition name="fade" mode="out-in">
        <p :key="game?.usName" class="left-14 teamName" id="team1name">
          {{
            game?.usName
              ? game?.usName
    ...
    <div class="left-[621px] teamWrap" ref="team2wrapper">
      ...
      <img
        ...
        class="absolute w-[66px] h-[62px] -top-0.5 left-[5px]" />
    </div>
  </div>
</template>
...
.score {
  @apply absolute text-[1.9rem] w-[55px] h-[62px] flex justify-center items-center top-0;
