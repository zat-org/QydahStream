  ...
  <div>
    ...
    <div class="left-[580px] teamWrap" ref="team1wrapper">
      ...
      <transition name="fade" mode="out-in">
        <p :key="game?.usName" class="left-[80px] teamName">
          {{
            game?.usName
              ? game?.usName
    ...
    <div class="left-[180px] teamWrap" ref="team2wrapper">
      <p class="-right-[7px] score">
        {{ !sakka_ended ? last_sakka?.themSakkaScore : game?.themGameScore }}
      </p>
      <transition name="fade" mode="out-in">
        <p :key="game?.themName" class="teamName left-[70px]">
          {{
            game?.themName
              ? game?.themName
...
.teamName {
  @apply absolute w-[185px]   text-[2rem] h-[40px] flex justify-center items-center top-1.5;
}

.teamWrap {
  @apply text-[white] text-center w-[324px] h-[62px] absolute opacity-100 top-[201px];
}
